...
const topDeliverThreeMenIntoDB = async () => {
  ...
  authData.forEach((auth) => {
    if (auth.role === 'DeliverMan') {
      ...
      parcelData.forEach((parcel) => {
        if (
          parcel.deliveryMan === auth.authId &&
  ...
  // Flatten listDeliverMen into an array of key-value pairs for easier
...
const statisticsAuthParcelFromDB = async (id: string) => {
  ...
  if (authData.role === 'User') {
    ...
    const reviewData = await UserReviewModel.findOne({
      deliverManId: authData.authId,
    });
