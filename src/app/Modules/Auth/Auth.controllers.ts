...
const authAccount = catchAsync(async(req, res)=>{
    const authData = await AuthServices.authAccountIntoDB(req.params.email, req.body)

    const tokenData = await AuthServices.createJwtToken({email:authData.email})
...
const addUserReview = catchAsync(async(req, res)=>{
    const result = await AuthServices.addUserReviewIntoDB(req.params.id, req.body);
    sendResponse(res, {
        statusCode: 200,
        success: true,
...
const authRoleChange = catchAsync(async(req, res)=>{
    ...
    const result = await AuthServices.authRoleChangeIntoDB(req.params.id, req.body);
