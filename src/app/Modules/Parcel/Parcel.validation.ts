...
const createParcelValidationSchema = z.object({
    body: z.object({
        ...
        parcelWeight: z.number().positive('Parcel weight must be a positive number'),
        price: z.number().positive('Price must be a positive number'),
        receiverName: z.string().min(1, 'Receiver name is required'),
        receiverPhoneNumber: z.string().min(1, 'Receiver phone number is required'),
        parcelDeliveryAddress: z.string().min(1, 'Parcel delivery address is required'),
        requestedDeliveryDate: z.string(),
        deliveryAddressLatitude: z.string().min(1, 'Delivery address latitude is required'),
        deliveryAddressLongitude: z.string().min(1, 'Delivery address longitude is required'),
    })
});
const updateParcelValidationSchema = z.object({
    body: z.object({
        ...
        parcelWeight: z.number().positive('Parcel weight must be a positive number').optional(),
